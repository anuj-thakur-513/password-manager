...
const handleSignupManual = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!user) {
        throw new ApiError(500, "Error while registering user, please try again later");
    }
...
const handleRefreshTokens = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
        decodedToken = jwt.verify(token, config.jwt.jwtSecret as Secret) as JwtPayload;
    } catch (error: any) {
        throw new ApiError(401, "Invalid Refresh Token");
    }

    const user = await User.findById(decodedToken.userId).select("-createdAt -updatedAt -password");
    if (!user) {
        throw new ApiError(404, "User not found");
    }
