...
const verifyToken = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  async (req: Request, res: Response, next: NextFunction) => {
    const token: string = req?.cookies?.accessToken || req.header("Access-Token")?.replace("Bearer", "");
    ...
    try {
      ...
      const user = await User.findOne({ _id: decodedToken.userId }).select("-createdAt -updatedAt -googleId");
      if (!user) {
      ...
      if (error.name === "TokenExpiredError" || error.statusCode === 401) {
        ...
        try {
          ...
          const { accessToken, refreshToken } = generateTokens(new Types.ObjectId(decodedToken.userId));
